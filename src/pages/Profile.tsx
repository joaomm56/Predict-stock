...
const Profile = () => {
  ...
  const handlePasswordSave = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setPwError(t.profile.pw_mismatch);
      return;
    }
    if (newPassword.length < 6) {
      setPwError(t.profile.pw_min);
      return;
    }
      ...
      <div className="container mx-auto px-6 pt-28 pb-20 max-w-2xl">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 border border-primary/20 px-4 py-1.5 mb-4">
            ...
            <span className="font-mono text-xs text-primary">{t.profile.badge}</span>
          </div>
          <h1 className="text-3xl font-bold text-foreground">{t.profile.title} <span className="text-primary">{t.profile.title_hl}</span></h1>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="font-semibold text-foreground">{t.profile.info_title}</h2>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-3 rounded-lg bg-secondary px-4 py-3">
                <Mail className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                <div>
                  <p className="text-xs text-muted-foreground">{t.profile.lbl_email}</p>
              ...
              <div className="flex items-center gap-3 rounded-lg bg-secondary px-4 py-3">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">{t.profile.lbl_plan}</p>
              ...
              {isGoogleUser && (
                <div className="flex items-center gap-3 rounded-lg bg-secondary px-4 py-3">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground">{t.profile.lbl_login_method}</p>
          ...
          >
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="font-semibold text-foreground">{t.profile.name_title}</h2>
            </div>
            <form onSubmit={handleNameSave} className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {nameLoading ? t.profile.btn_saving : t.profile.btn_save_name}
          ...
          {!isGoogleUser && (
            ...
            >
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="font-semibold text-foreground">{t.profile.pw_title}</h2>
              </div>
              <form onSubmit={handlePasswordSave} className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t.profile.ph_new_pw}
                    required
                    minLength={6}
                    className="w-full rounded-lg bg-secondary border border-border pl-10 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-colors"
                  />
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t.profile.ph_confirm_pw}
                    required
                    className="w-full rounded-lg bg-secondary border border-border pl-10 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-colors"
                  />
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 transition-all disabled:opacity-50"
                  >
                    {pwLoading ? t.profile.btn_saving : t.profile.btn_change_pw}
                    <ArrowRight className="h-3.5 w-3.5" />
                  </button>
                </div>
              </form>
